#!/usr/bin/env python3
"""Build four canonical v0.140 SFD sources with residual glyph completion."""
...
def main() -> None:
    ...
    script = root / "scripts" / "develop_serif_v140.py"
    for style in ("Regular", "Bold", "Italic", "BoldItalic"):
        source = iterations / f"TishteSerif-{style}-v130.sfd"
        output = iterations / f"TishteSerif-{style}-v140.sfd"
        arguments = [str(path.relative_to(root)) for path in (script, source, output)]
        subprocess.run(
            [str(find_fontforge()), "-lang=py", "-script", *arguments, "0.140"],
            cwd=root,
            check=True,
        )
        normalize(output)
